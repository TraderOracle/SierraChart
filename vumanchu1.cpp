...
SCDLLName("VuManChu Cipher B Divergences")
...
//  Study
//============================================================================
SCSFExport scsf_VuManChuCipherBDivergences(SCStudyInterfaceRef sc)
{
    //------------------------------------------------------------------ plots
    ...
    if (sc.SetDefaults)
    {
        sc.GraphName        = "VMC Cipher_B_Divergences";
        sc.GraphRegion      = 1;
        sc.AutoLoop         = 1;
        sc.ValueFormat      = 2;
        sc.FreeDLL          = 0;
    ...
    //====================================================================
    //  Alerts  (Sierra Chart: set an alert condition on the study, or use
    //  the code below with sc.SetAlert / sc.AddMessageToLog)
    //====================================================================
    if (sc.Index == sc.ArraySize - 1 && sc.IsFullRecalculation == 0)
    {
        if (BuySignal)
            sc.SetAlert(0, "VMC Cipher B: Buy (big green circle)");
        else if (SellSignal)
            sc.SetAlert(0, "VMC Cipher B: Sell (big red circle)");
        else if (GoldBuy)
            sc.SetAlert(0, "VMC Cipher B: GOLD buy");
    }
}
