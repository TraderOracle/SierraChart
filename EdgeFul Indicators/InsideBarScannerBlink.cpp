...
SCSFExport scsf_InsideBarScanner(SCStudyInterfaceRef sc)
{
	if (sc.SetDefaults)
	{
		...
		sc.Input[IN_BLINK].Name = "Blink Text";
		sc.Input[IN_BLINK].SetYesNo(1);

		sc.Input[IN_BLINK_ALL].Name = "Blink All Lines (No = only inside bar hits)";
		sc.Input[IN_BLINK_ALL].SetYesNo(0);

		sc.Input[IN_BLINK_SECONDS].Name = "Blink Interval (seconds)";
		sc.Input[IN_BLINK_SECONDS].SetInt(1);
		sc.Input[IN_BLINK_SECONDS].SetIntLimits(1, 30);
	...
	// Blink phase.
	//
	// A blink is just the text being drawn on one pass and erased on the next.
	// The study has to keep getting called for that to happen, which is what
	// sc.UpdateAlways does. The toggle is derived from the wall clock rather
	// than a counter, so every line flashes in unison and the phase survives a
	// recalculation.
	//
	// The fastest possible blink is bounded by Global Settings >> General
	// Settings >> Chart Update Interval (500 ms by default).
	//------------------------------------------------------------------------
	const int BlinkEnabled  = sc.Input[IN_BLINK].GetYesNo();
	const int BlinkAllLines = sc.Input[IN_BLINK_ALL].GetYesNo();

	int BlinkSeconds = sc.Input[IN_BLINK_SECONDS].GetInt();
	if (BlinkSeconds < 1)
		BlinkSeconds = 1;

	sc.UpdateAlways = BlinkEnabled ? 1 : 0;

	const SCDateTime Now = sc.CurrentSystemDateTime;
	const int SecondOfDay = Now.GetHour() * 3600 + Now.GetMinute() * 60 + Now.GetSecond();
	const int BlinkVisible = (!BlinkEnabled) || (((SecondOfDay / BlinkSeconds) % 2) == 0);

	const SCString BlankText = "";
	...
	for (int i = 0; i < MAX_SYMBOLS; ++i)
	{
		...
		if (Result.IsInsideBar)
		{
			...
			SetTextLine(sc, NextLine++, BlinkVisible ? Text : BlankText, HitColor);
		}
		else if (ShowNonHits)
		{
			Text.Format("%s   -",
				GetChartLabel(sc, ChartNumber).GetChars());
			SetTextLine(sc, NextLine++,
				(BlinkVisible || !BlinkAllLines) ? Text : BlankText, MissColor);
		}
	}

	Text.Format("INSIDE BAR SCAN - prev completed daily - %d of %d", HitCount, Scanned);
	SetTextLine(sc, 0, (BlinkVisible || !BlinkAllLines) ? Text : BlankText, HeaderColor);
