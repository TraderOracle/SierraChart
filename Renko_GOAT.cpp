...
#pragma endregion
	...
	if (sc.ChartDrawingExists(sc.ChartNumber, 5))
		sc.DeleteUserDrawnACSDrawing(sc.ChartNumber, 5);

	if (i < sc.ArraySize - 1)
	{
		int Transparency_Level = 70;
		int Rectangle_Line_Width = 30;
		int UniqueLineNumber = 5;
		Tool.Clear();
		Tool.ChartNumber = sc.ChartNumber;
		Tool.DrawingType = DRAWING_RECTANGLEHIGHLIGHT;
		Tool.DrawUnderneathMainGraph = true;
		Tool.LineNumber = UniqueLineNumber;
		Tool.BeginDateTime = sc.BaseDateTimeIn[1];
		Tool.BeginIndex = 1;
		Tool.BeginValue = 20000;
		Tool.EndIndex = sc.Index;
		Tool.EndValue = 20040;
		Tool.Color = RGB(0, 200, 200);
		Tool.SecondaryColor = RGB(0, 200, 200);
		Tool.TransparencyLevel = Transparency_Level;
		Tool.Text = "howdy";
		Tool.TextAlignment = DT_RIGHT || DT_VCENTER;
		Tool.AddMethod = UTAM_ADD_OR_ADJUST;
		Tool.ExtendLeft = true;
		Tool.ExtendRight = true;
...
SCSFExport scsf_TraderSmarts(SCStudyInterfaceRef sc)
{
	...
	SCInputRef Input_ShowTS = sc.Input[1];
	SCInputRef Input_ShowMTS = sc.Input[2];
	SCInputRef Input_Paragraph = sc.Input[3];
	...
	if (sc.SetDefaults)
	{
		...
		Input_FileName.Name = "Filename to store data";
		Input_FileName.SetPathAndFileName("C:\\temp\\tradersmarts.txt");

		Input_ShowTS.Name = "Filena5me to store data";
		Input_ShowTS.SetYesNo(SC_NO);

		Input_ShowMTS.Name = "Fi5lename to store data";
		Input_ShowMTS.SetYesNo(SC_YES);

		Input_Paragraph.Name = "Filenam51e to store data";
		Input_Paragraph.SetString("");
			...
			if (strstr(ss, "Sand") || strstr(ss, "Long") || strstr(ss, "Short"))
			{
				...
				if (id > 0)
				{
					...
					if (i > 0)
					{
						...
						//sc.AddMessageToLog(w.Format("Sand = %f", fStart), 1);
						//vStart.push_back(fStart);
					}
				}
				else
				{
					...
					if (i > 0)
					{
						...
						vStart.push_back(fStart);
						vEnd.push_back(fEnd);
						desc = ss.GetSubString(ss.GetLength() - i - 1, i + 1);
	...
	if (i < sc.ArraySize - 1)
	{
		sc.AddMessageToLog("Displaying lines", 1);
		int x = 0;
		for (const float& af : vEnd)
		{
			sc.AddMessageToLog(w.Format("%s - %f", vDesc.at(x).GetChars(), vEnd.at(x)), 1);
			s_UseTool Tool;
			Tool.Clear();
			Tool.ChartNumber = sc.ChartNumber;
			Tool.DrawingType = DRAWING_RECTANGLEHIGHLIGHT;
			...
			Tool.BeginDateTime = sc.BaseDateTimeIn[1];
			Tool.BeginIndex = 1;
			Tool.BeginValue = vStart.at(x);
			Tool.EndIndex = sc.Index;
			Tool.EndValue = vEnd.at(x);
			Tool.Color = RGB(0, 200, 200);
			Tool.SecondaryColor = RGB(0, 200, 200);
			Tool.TransparencyLevel = Transparency_Level;
			Tool.Text = vDesc.at(x);
			Tool.TextAlignment = DT_RIGHT || DT_VCENTER;
			Tool.AddMethod = UTAM_ADD_OR_ADJUST;
			Tool.ExtendLeft = true;
			Tool.ExtendRight = true;
