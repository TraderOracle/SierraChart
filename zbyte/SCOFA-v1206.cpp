...
//
// v1206 edge-tracking & run fixes (docs/scofdoc-11-v1206-refinements.md):
//   Fix E1 — edge tracking (MFE/MAE folding, target/stop resolution)
//            gated on closed bars like the detectors; with In:30=No
//            it previously sampled only the forming bar's first tick
//            and the per-bar guard then skipped the closed bar
//            forever (understated MFE/MAE, premature resolutions,
//            live-vs-recalc edge-stat divergence)
//   Fix E2 — "sweep wins" ruling (2026-07-06): a liquidity RUN
//            resolving on a bar whose fresh-raid scan emits the
//            opposite-direction SWEEP is suppressed — the deeper
//            pool's same-bar reclaim contradicts the run's
//            no-reclaim acceptance narrative
//   Fix E3 — the historical-backfill edge summary also fires when
//            the final recalc bar is session-filtered or still
//            forming (previously unreachable on a live chart)
//   E4  — EvaluateSweepRaidBar measures before it judges: pends
//         built from gate-failing raid bars carry real VAP metrics,
//         so RUN strength's aligned-delta term is live. Verdict and
//         absorption/sweep/slingshot signal sets are unchanged.
//
...
#define SCOFA_VERSION_STR  "1.2.06"
#define SCOFA_VERSION_NUM  "1206"

SCDLLName("SCOFA-v" SCOFA_VERSION_NUM)

static const int   STUDY_VERSION      = 1206;   // must equal SCOFA_VERSION_NUM
...
SCSFExport scsf_SCOFAbsorptionDetector(SCStudyInterfaceRef sc)
{
            ...
            In_EndHour.GetInt(),   In_EndMinute.GetInt()))
        {
            if (s.enableRects && !state.Zones.empty())
                UpdateExtendingRectangles(sc, state.Zones, sc.Index,
                    ...
                    s.zoneTransparency);
            // Fix E3 (v1206): the backfill summary must not be lost
            // when the final recalc bar falls outside the session.
            if (s.edgeEnabled && sc.IsFullRecalculation
        ...
        if (sc.GetBarHasClosedStatus(sc.Index) != BHCS_BAR_HAS_CLOSED)
        {
            if (s.enableRects && !state.Zones.empty())
                UpdateExtendingRectangles(sc, state.Zones, sc.Index,
                    s.maxExtBars, s.bullZoneColor, s.bearZoneColor,
                    s.zoneTransparency);
            // Fix E3 (v1206): on a live chart the final recalc bar
            // is still forming, so this return made the backfill
            // summary unreachable with In:30=Yes.
            if (s.edgeEnabled && sc.IsFullRecalculation
                && sc.Index == sc.ArraySize - 1)
                LogEdgeSummary(sc, state, s, "historical backfill");
            return;
    ...
    // Fix E1 (v1206): edge tracking joins this gate — see below.
    // ----------------------------------------------------------
    ...
    // Fix E1 (v1206): gated on barClosed. With In:30=No this
    // previously ran on the forming bar's FIRST tick, folded the
    // partial High/Low into MFE/MAE and the per-bar guard then
    // skipped the bar's closed reprocessing forever — understating
    // MFE/MAE, resolving signals prematurely (a first-tick spike
    // could log TARGET where the closed bar's stop-first rule says
    // STOP) and diverging from the full-recalc statistics. With
    // In:30=Yes (default) the global bar-close gate above already
    // returned for forming bars, so behavior is unchanged.
    // ----------------------------------------------------------
    if (s.edgeEnabled && barClosed)
...
// v1206 (E4): measure-then-verdict — every out-param is
// populated even when a gate fails, so a pend built from a
// failing raid bar carries real metrics for RUN grading. The
// returned verdict is unchanged for every input.
// ============================================================
    ...
    double& out_zoneDelta, double& out_penZoneVol)
{
    ...
    // E4 (v1206): measurements first, verdict last. A failing gate
    // no longer skips the remaining measurements, so a PendingSweep
    // built from this bar carries real burst/delta/volume metrics
    // even when CondsPassed = false. The only consumer of that case
    // is liquidity-RUN grading, whose 15-point aligned-delta term
    // was structurally dead (ZonePenVolume stayed 0 for exactly the
    // low-wick raid bars runs reward). Gate thresholds and the
    // returned verdict are identical to v1205.

    // Condition 3: rejection wick. Auto-fail on zero-range bars
    // (wick % undefined; the measurement stays 0).
    bool wickOk = false;
    if (range >= halfTick)
    {
        ...
        wickOk = (out_wickPct + FP_EPSILON
                      >= static_cast<float>(s.sweepMinWickPct));
    }
    ...
    if (nPen >= 1 && nAll >= 1 && volAll > 0.0)
    {
        ...
        if (baseN >= 1 && baseVol > 0.0)
        {
            ...
            burstOk = (out_burstPct + FP_EPSILON
                           >= static_cast<float>(s.sweepBurstPct));
        }
    }
    ...
    {
        deltaOk = isBullish
            ? (zoneDelta <= -static_cast<double>(s.sweepMinZoneDelta))
            : (zoneDelta >=  static_cast<double>(s.sweepMinZoneDelta));
    }

    return wickOk && burstOk && deltaOk;
}
...
// Fix E2 (v1206): when the resolved window yields a RUN and the
// fresh-raid scan then emits a same-bar sweep of a deeper pool,
// the sweep wins and the run is suppressed (user ruling).
// ============================================================
    ...
    const StudySettings& s, StudyState& state, DetectionResult& out_run)
{
    ...
    for (int c = 0; c < nRaided; ++c)
    {
        ...
        if (reclaimedSameBar)
        {
            ...
            // Fix E2 (v1206, "sweep wins" ruling): if this bar also
            // resolved the reclaim window as an opposite-direction
            // RUN through a nearer pool, suppress the run — the
            // same-bar reclaim of the deeper pool contradicts the
            // run's no-reclaim acceptance narrative. Detection and
            // pool consumption are unaffected; only the emission is
            // withdrawn.
            out_run.Detected = false;
